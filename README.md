# アパレル系売上在庫データ分析用DB (PostgreSQL)

## 概要
本プロジェクトは、アパレル業界を想定したデータベースを設計し、  
売上（EC / 店舗）と在庫データを統合して分析を行うポートフォリオです。  
単なるCRUDではなく、実務を想定したデータモデル設計と分析SQLの実装を目的としています。  

## 目的
・SQLを用いた初めてのDB構築を試作して構造を理解する  
・服飾・広告業界のデータ分析に興味があるのでまずは在庫管理の簡易的なDBを試作してみることにしました。  

## 使用技術
\* PostgreSQL  
\* SQL（JOIN / GROUP BY / UNION ALL / サブクエリ / 集計関数）  
\* pgAdmin  
\* CHATGPT（手詰まり箇所の検索に使用）
...
\* PythonによるETL処理の追加  
## 感想
・とりあえずDB構築の全体像を把握して「どのようにDBを構築するか」「どんな項目を入力させるか」一連の基礎的な構築の流れを掴んでみた。
・まだ分析用データを入れるためだけの設計という感じなので、実際にPythonを使用してグラフ化などもできるか確認してみたくなった。
・今後はAWSとの連携、APIとの連携、より実用的なアイデアを基にして構築できるか挑戦するのが課題。  
